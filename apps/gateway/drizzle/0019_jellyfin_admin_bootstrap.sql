...
CREATE TABLE `__new_jellyfin_quick_connect_transactions` (
	...
	CONSTRAINT "jellyfin_quick_connect_transactions_purpose_check" CHECK(("__new_jellyfin_quick_connect_transactions"."purpose" = 'sign_in' and "__new_jellyfin_quick_connect_transactions"."pairing_session_id" is null)
        or ("__new_jellyfin_quick_connect_transactions"."purpose" in ('pairing', 'bootstrap') and "__new_jellyfin_quick_connect_transactions"."pairing_session_id" is not null)),
...
INSERT INTO `__new_jellyfin_quick_connect_transactions`("id", "connector_id", "connector_type", "purpose", "pairing_session_id", "browser_binding_hash", "encrypted_payload", "expires_at", "next_poll_at", "poll_count", "consumed_at", "created_at") SELECT "id", "connector_id", "connector_type", "purpose", "pairing_session_id", "browser_binding_hash", "encrypted_payload", "expires_at", "next_poll_at", "poll_count", "consumed_at", "created_at" FROM `jellyfin_quick_connect_transactions`;--> statement-breakpoint
