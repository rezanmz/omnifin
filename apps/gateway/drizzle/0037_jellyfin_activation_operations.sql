CREATE TABLE `jellyfin_activation_operations` (
	...
	CONSTRAINT "jellyfin_activation_operations_state_check" CHECK("jellyfin_activation_operations"."state" in ('reserved', 'create_dispatched', 'created_id_recorded', 'manual_required', 'tombstoned')),
	...
	CONSTRAINT "jellyfin_activation_operations_attempt_check" CHECK("jellyfin_activation_operations"."create_attempt_count" between 0 and 1 and "jellyfin_activation_operations"."retry_count" between 0 and 8 and "jellyfin_activation_operations"."cleanup_attempt_count" between 0 and 8),
...
CREATE UNIQUE INDEX `jellyfin_activation_operations_invitation_user_identity_unique` ON `jellyfin_activation_operations` (`invitation_id`,`user_id`,`external_identity_id`);--> statement-breakpoint
...
ALTER TABLE `service_identity_links` ADD `provisioned_by_activation_id` text REFERENCES jellyfin_activation_operations(id);--> statement-breakpoint
