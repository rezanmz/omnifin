...
import {
  ...
  type SystemStatusSnapshotEvent,
} from "@omnifin/contracts/system";
import { performance } from "node:perf_hooks";

import { randomToken } from "../security/crypto.js";
import type { SystemStatusContext } from "./status-service.js";
...
interface SystemStatusEventReader {
  read(context: SystemStatusContext, signal?: AbortSignal): Promise<SystemStatusResponse>;
}
...
export interface SystemStatusEventSubscriber {
  lastEventId?: string;
  onClose(): void;
  onEvent(event: SystemStatusSnapshotEvent): void;
...
export class SystemStatusEventBroker {
  ...
  readonly #reader: SystemStatusEventReader;
  ...
  public constructor(
    ...
    dependencies: SystemStatusEventBrokerDependencies = {},
  ) {
    this.#reader = reader;
  ...
  #broadcast(event: SystemStatusSnapshotEvent) {
  ...
  #deliver(subscriber: RetainedSubscriber, event: SystemStatusSnapshotEvent) {
    try {
  ...
  async #run(controller: AbortController) {
    try {
      while (!controller.signal.aborted && this.#subscribers.size > 0) {
        ...
        if (!principal) return;
        const status = await this.#reader.read({ principal }, controller.signal);
        if (controller.signal.aborted) return;
        const event = systemStatusSnapshotEventSchema.parse({
