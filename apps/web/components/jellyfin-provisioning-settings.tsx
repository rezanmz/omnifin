...
function safeError(error: unknown) {
  if (!(error instanceof ConnectorAdminClientError)) return "The provisioning settings could not be loaded.";
  ...
  if (error.code.includes("revision_conflict")) return "These settings changed elsewhere. Refresh to reconcile the latest version.";
  return error.message || "The operation could not be completed. No unconfirmed settings were applied.";
}
...
export function JellyfinProvisioningSettings({ connector, csrfToken, client = jellyfinProvisioningClient }: JellyfinProvisioningSettingsProperties) {
  ...
  const save = async (credential: JellyfinProvisioningCredentialWrite, nextEnabled = enabled) => {
    if (!config) return;
    setBusy("save"); setError(null); setNotice(null);
    try {
      const next = await client.update(connector.id, { credential, enabled: nextEnabled && canEnable, revision: config.revision, templateUserId: selectedTemplate?.id ?? null }, csrfToken);
      ...
      if (reason instanceof ConnectorAdminClientError && reason.code.includes("revision_conflict")) {
        try { const fresh = await client.get(connector.id); setConfig(fresh); setEnabled(fresh.enabled); setTemplateUserId(fresh.template?.id ?? ""); } catch { /* keep the actionable conflict message */ }
      }
  ...
  return <section className={styles.provisioning} aria-labelledby="provisioning-title">
    ...
    {error ? <div className={styles.formError} role="alert"><CircleAlert aria-hidden="true" size={16} /> {error}<button className={styles.inlineAction} onClick={() => window.location.reload()} type="button">Refresh</button></div> : null}
    {config ? <>
      <div className={styles.provisioningGrid}>
        ...
        <div className={styles.templatePanel}>
          ...
          <button className={styles.secondaryButton} disabled={!canLoadTemplates || busy !== null} onClick={() => void loadTemplates()} type="button">{busy === "templates" ? <LoaderCircle className={styles.spinner} size={15} /> : <RefreshCw size={15} />} {templates.length ? "Refresh templates" : "Load templates"}</button>
      ...
      {credentialExists ? <div className={styles.provisioningDanger}>{confirmClear ? <div role="group" aria-label="Confirm clearing provisioning credential"><span>Clear the sealed credential and disable provisioning?</span><button className={styles.secondaryButton} onClick={() => setConfirmClear(false)} type="button">Keep credential</button><button className={styles.dangerButton} disabled={busy !== null} onClick={() => { setConfirmClear(false); void save({ kind: "clear" }, false); }} type="button"><Trash2 size={15} /> Clear credential</button></div> : <button className={styles.dangerButton} disabled={busy !== null} onClick={() => setConfirmClear(true)} type="button"><Trash2 size={15} /> Clear credential</button>}</div> : null}
