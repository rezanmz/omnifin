...
`request.create`, live identity-and-target-scoped Radarr/Sonarr title provenance, exact stalled-item
queue recovery, and exact whole-title monitoring for principals with `acquisition.manage`. It also
exposes permission-gated qBittorrent and SABnzbd queue
...
reporting, Bazarr subtitle search and download, and guarded Jellyfin library maintenance. The
protected live compatibility baseline, broader acquisition mutations, and release-grade live
compatibility evidence remain unavailable.
